...
abstract Philox{T<:Union{UInt32, UInt64}} <: AbstractRNG{T}
...
"""
type Philox2x{T<:Union{UInt32, UInt64}, R} <: Philox{T}
    key::T
    ctr1::T
    ctr2::T
    x1::T
...
function Philox2x{T<:Union{UInt32, UInt64}}(::Type{T}=UInt64, seed::Integer=gen_seed(T), R::Integer=10)
    @assert 1 <= R <= 16
    Philox2x{T, Int(R)}(seed % T, 0 % T, 0 % T, 0 % T, 0 % T)
end
...
@inline function rand{T<:Union{UInt32, UInt64}}(r::Philox2x{T}, ::Type{T})
    philox_r(r)
    r.ctr1 += 1 % T
    r.x1
end
...
@inline function philox_r{T<:Union{UInt32, UInt64}, R}(r::Philox2x{T, R})
...
"""
type Philox4x{T<:Union{UInt32, UInt64}, R} <: Philox{T}
    key1::T
    key2::T
    ctr1::T
    ctr2::T
...
function Philox4x{T<:Union{UInt32, UInt64}}(::Type{T}=UInt64,
    ...
    @assert 1 <= R <= 16
    Philox4x{T, Int(R)}(seed[1] % T, seed[2], 0 % T, 0 % T, 0 % T, 0 % T, 0 % T, 0 % T, 0 % T, 0 % T)
end
...
@inline function rand{T<:Union{UInt32, UInt64}}(r::Philox4x{T}, ::Type{T})
    philox_r(r)
    r.ctr1 += 1 % T
    r.x1
end
...
@inline function philox_r{T<:Union{UInt32, UInt64}, R}(r::Philox4x{T, R})
